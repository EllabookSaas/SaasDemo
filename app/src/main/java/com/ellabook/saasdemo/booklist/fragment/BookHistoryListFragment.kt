 ...
 */
class BookHistoryListFragment : Fragment(), FragmentKeyDownEvent {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        //获取/观察数据
        bookHistoryListViewModel =
            ViewModelProvider(requireActivity()).get(BookHistoryListViewModel::class.java)

        bookHistoryListViewModel.bookHistoryList.observe(viewLifecycleOwner) {
            adapter.updateData(it.books, if (it.returnTop) bookListView else null)
        }

        bookHistoryListViewModel.initHistoryData()

        view.findViewById<CustomEditText>(R.id.search).onTextWatchListener = OnTextWatchListener {
            bookHistoryListViewModel.searchData(it)
        }
    }
